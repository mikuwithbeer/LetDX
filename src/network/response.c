...
let_size_t let_network_response_to_bytes(const let_network_response_t *response,
    ...
    switch (response->id) {
        ...
        case LET_NETWORK_RESPONSE_ID_ADD_ACCOUNT:
            written_bytes = sprintf(output, "AID %llX", response->data.add_account);
            break;
        case LET_NETWORK_RESPONSE_ID_GET_BALANCE:
            ...
            if (balance_high == 0) {
                written_bytes = sprintf(output, "BAL %llX", balance_low);
            } else {
                written_bytes = sprintf(output, "BAL %llX%016llX", balance_high, balance_low);
            }
        ...
        case LET_NETWORK_RESPONSE_ID_ERROR: {
            ...
            written_bytes = sprintf(output, "ERR %hX", error_code);
        }
    }
